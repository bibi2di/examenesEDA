...
public class DoubleLinkedList<T> {
    ...
    public void borrarLista(DoubleLinkedList<T> subLista) {
        ...
        if (subLista.first == null) {} // la lista vacía está contenida en cualquier otra lista. No hay que
        // eliminar nada
        else {
            DoubleNode actual = first;
            while(!actual.data.equals(subLista.first.data)){
                actual = actual.next;
            }
            DoubleNode primero = actual;
            DoubleNode actual2 = subLista.first;
            while (actual2.next != subLista.first){
                actual2 = actual2.next;
                actual = actual.next;
            }
            DoubleNode ultimo = actual;
            if (primero == first && first.prev == ultimo){first = null;}
            else{
            primero.prev.next = ultimo.next;
            ultimo.next.prev = primero.prev;}
